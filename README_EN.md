...
| [`software-ui-design`](skills/software-ui-design/SKILL.md) | Guides a complete UI design workflow from 0 to 1 for users unfamiliar with design: goal definition, user & scenario research, competitor reference, scope convergence, information architecture, core task flows, low-fi layout, interaction & four-state design, visual design system, and testing — mining each stage with concrete questions and self-checks, producing a structured UI design document plus an interactive HTML prototype that visualizes the design instead of relying on text alone | `UI设计` `帮我设计界面` `带我走一遍UI设计流程` `UI设计文档` `@software-ui-design` |
